 ...
 * @param frame: Specific frame selected to be cleared of data
 *
 **/
void CanClearTxDataFrame(CanDataFrameInit *frame)
{
    frame->tx_header.StdId = 0x00;
    frame->rx_header.ExtId = 0x00;
    frame->tx_header.RTR = CAN_RTR_DATA;
    frame->tx_header.IDE = CAN_ID_STD;
    frame->tx_header.DLC = 0;
    frame->tx_header.TransmitGlobalTime = DISABLE;
    frame->tx_data[0] = 0x0;
    frame->tx_data[1] = 0x0;
    frame->tx_data[2] = 0x0;
    frame->tx_data[3] = 0x0;
...
void CanClearRxDataFrame(CanDataFrameInit *frame)
{
    frame->rx_header.StdId = 0x00;
    frame->rx_header.ExtId = 0x00;
    frame->rx_header.RTR = CAN_RTR_DATA;
    frame->rx_header.IDE = CAN_ID_STD;
    frame->rx_header.DLC = 0;
    frame->rx_data[0] = 0x0;
    frame->rx_data[1] = 0x0;
    frame->rx_data[2] = 0x0;
    frame->rx_data[3] = 0x0;
...
int CanSendStandardIdMessage(uint32_t FrameId, uint8_t DLC,
                             ...
                             uint8_t byte4, uint8_t byte5, uint8_t byte6, uint8_t byte7)
{
    can_frame_template.tx_header.StdId = FrameId;
    can_frame_template.tx_header.RTR = CAN_RTR_DATA;
